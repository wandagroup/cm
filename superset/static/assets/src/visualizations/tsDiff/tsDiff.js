...
import d3 from 'd3';
import * as d3v4 from './d3v4.min.js'; 
import './tsDiff.css';
import './d3_timeseries.css';
...
function d3_timeseries()  {
  ...
  '<td style="color:#333333;text-align:right">' + yscale.setformat(d.item[d.aes.y]) + '</td></tr>';
    ...
    return '<h4>' + xscale.setformat(d3v4.timeDay(date)) + '</h4>' + spans;
  };
  ...
  function createLines(serie) {
    ...
    serie.interpolationFunction = d3v4[curveName] || d3v4.curveLinear;
    ...
    serie.find = function (date) {
      var bisect = d3v4.bisector(fk(aes.x)).left;
  ...
  function mouseMove() {
    var x = d3v4.mouse(container.node())[0];
    x = xscale.invert(x);
  ...
  var chart = function (elem)  {
    ...
    series = series.map(function (s) {
      var extent = d3v4.extent(s.data.map(functorkey(s.aes.y)));
      s.min = extent[0];
      s.max = extent[1];
      extent = d3v4.extent(s.data.map(functorkey(s.aes.x)));
      s.dateMin = extent[0];
      s.dateMax = extent[1];
      return s;
    });
    ...
    yscale.range([height - margin.top - margin.bottom - drawerHeight - drawerTopMargin, 0])
      .domain([d3v4.min(series.map(fk('min'))),
               d3v4.max(series.map(fk('max')))])
      .nice();


    xscale.range([0, width - margin.left - margin.right])
      .domain([d3v4.min(series.map(fk('dateMin'))),
               d3v4.max(series.map(fk('dateMax')))])
      .nice();
    ...
    // create svg
    svg = d3v4.select(elem).append('svg')
      .attr('width', width)
      .attr('height', height);
    ...
    var xAxis = d3v4.axisBottom().scale(xscale).tickFormat(xscale.setformat);
    var yAxis = d3v4.axisLeft().scale(yscale).tickFormat(yscale.setformat);

    brush.extent([[fullxscale.range()[0], 0], [fullxscale.range()[1], drawerHeight - drawerTopMargin]])

      .on('brush', () => {
        let selection = d3v4.event.selection;
    ...
    svg.append('g')
      ...
      .attr("x", -margin.top - d3v4.mean(yscale.range()))
      .attr("dy", ".71em")
  ...
  d3v4.keys(margin).forEach(function (k) {
...
/*function createRandomData(n, range, rand) {
  ...
  var data = [];
  var rgen = d3v4.randomNormal(0, (range[1] - range[0]) * rand);
  for (var i = 0; i < n; i++) {
    data.push({
      date: d,
      n: num,
...
function tsDiff(element, props) {
  ...
  console.log('props is as follows:');
  console.log(props);

  //element.innerHTML = '';
  element.innerHTML = '<div id="tsDiffChart"></div>';
  const div = d3.select(element);
  const { colNames,matrix } = data;
  /*console.log('colNames is as follows');
  console.log(colNames);
  console.log(matrix);
  console.log('matrix[1].水温均值');*/
  console.log('height is as follows');
  console.log(height);
  //alert(new Date(data['date'][1]));

  //console.log('data2 are as follows');
  //console.log(data2);
  const chart = d3_timeseries()
              .addSerie(matrix,{x:eval('"'+colNames[2]+'"'),y:eval('"'+colNames[3]+'"'),diff:eval('"'+colNames[4]+'"')},{interpolate:'monotone',color:"#333"})
              .width(820)
  /*const chart = d3_timeseries()
              .addSerie(matrix,{x:'__timestamp',y:'水温均值',diff:'溶解氧均值'},{interpolate:'monotone',color:"#333"})
              .width(820)*/
  /*const data2 = createRandomData(80,[0,1000],0.01)
  const chart = d3_timeseries()
              .addSerie(data2,{x:'date',y:'n',diff:'n3'},{interpolate:'monotone',color:"#333"})
              .width(820)*/
...
tsDiff.displayName = 'tsDiff';
tsDiff.propTypes = propTypes;

export default tsDiff;
