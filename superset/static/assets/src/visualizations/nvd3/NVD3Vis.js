...
const propTypes = {
  ...
  // 'line-multi' or 'dual-line' or 'line-bar', or 'rwindrose'
  yAxis2Format: PropTypes.string,
  // 'line', 'time-pivot', 'dual-line' or 'line-multi' or 'rwindrose'
...
function nvd3Vis(element, props) {
  ...
  // noqa; used to test dashed line
  if (isVizTypes(['line'])) {
    for (let i = 0; i < data.length; i++) {
        if (data[i]['key'] === 'I~III类') {
  ...
  const drawGraph = function () {
    ...
    switch (vizType) {
      case 'line':
        if (canShowBrush) {     // noqa !
          ...
          chart = nv.models.lineChart();  // noqa linePlusBarChart(); -> lineChart();
        }
      ...
      case 'line_bar':
        chart = nv.models.multiChart().legendRightAxisHint('');
    ...
    if (isTimeSeries) {
      ...
      chart.interactiveLayer.tooltip.headerFormatter(formatDateVerbose);  // noqa uncomment
    } else {
    ...
    if (isVizTypes(['dual_line', 'rwindrose', 'rscatterplot', 'rcorplot', 'rtrajplot', 'rtheilsen', 'rpolarannulus', 'rpollutionrose', 'rtimevariation','line_multi', 'line_bar'])) {
      ...
      if (vizType === 'dual_line' || vizType === 'rwindrose' || vizType === 'rscatterplot' || vizType === 'rcorplot' || vizType ==='rtrajplot'  || vizType === 'rtheilsen' || vizType === 'rpolarannulus' || vizType === 'rpollutionrose' || vizType === 'rtimevariation' || vizType === 'line_bar') {
    ...
    if (isVizTypes(['dual_line', 'rwindrose', 'rscatterplot', 'rcorplot', 'rtrajplot', 'rtheilsen', 'rpolarannulus', 'rpollutionrose', 'rtimevation', 'line_multi', 'line_bar'])) {
    ...
    if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
      ...
      if (isVizTypes(['dual_line', 'rwindrose', 'rscatterplot', 'rcorplot', 'rtrajplot', 'rtheilsen', 'rpolarannulus', 'rpollutionrose', 'rtimevation', 'line_multi', 'line_bar'])) {
      //if (isVizTypes(['dual_line', 'rwindrose', 'line_multi', 'line_bar'])) {
